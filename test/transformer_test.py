...
class ModelTester:
    def __init__(self, model_path: str, device: str = None):
        ...
        self.test_dir = Path(__file__).parent / "results" / "transformer"
    ...
    def test(self, test_data_path: str, num_sequences: int = None):
        ...
        try:
            with h5py.File(test_data_path, 'r') as f:
                ...
                sequence_metrics_path = self.metrics_dir / f"{self.model.name}_sequence_metrics_{timestamp}.csv"
                overall_metrics_path = self.metrics_dir / f"{self.model.name}_overall_metrics_{timestamp}.csv"
                
                # Write headers
                with open(sequence_metrics_path, 'w') as seq_file:
                    seq_file.write('category,video_name,sequence,mse,ssim\n')
                ...
                for category in f.keys():
                    ...
                    for video_name in category_group.keys():
                        video_group = category_group[video_name]
                        
                        # Get input and target sequences
                        input_sequences = video_group['inputs'][()]
                        target_sequences = video_group['outputs'][()]
                        
                        for seq_idx in range(len(input_sequences)):
                            try:
                                # Load sequence
                                input_seq = torch.tensor(input_sequences[seq_idx], dtype=torch.float32).unsqueeze(0)
                                target_seq = torch.tensor(target_sequences[seq_idx], dtype=torch.float32).unsqueeze(0)
                                ...
                                # Get predictions
                                predictions, _ = self.model(input_seq, target_seq, training=False)
                ...
                overall_mse = np.mean(all_mse) if all_mse else float('nan')
                overall_ssim = np.mean(all_ssim) if all_ssim else float('nan')
                
                with open(overall_metrics_path, 'w') as f:
                    f.write('metric_type,category,mse,ssim\n')
                    f.write(f'overall,all,{overall_mse},{overall_ssim}\n')
                    
                    for category, metrics in category_metrics.items():
                        cat_mse = np.mean(metrics['mse']) if metrics['mse'] else float('nan')
                        cat_ssim = np.mean(metrics['ssim']) if metrics['ssim'] else float('nan')
                        f.write(f'category,{category},{cat_mse},{cat_ssim}\n')
...
def main():
    ...
    model_path = project_root / "models/SimpleTransformer_best.pth"
