...
class TransformerTester:
    ...
    def test(self, test_data_path: str, num_sequences: int = None):
        ...
        sequence_metrics_path = self.metrics_dir / f'sequence_metrics_transformer_{timestamp}.csv'
        overall_metrics_path = self.metrics_dir / f'overall_metrics_transformer_{timestamp}.csv'
                    ...
                    for video_name in category_group.keys():
                        ...
                        for seq_idx in range(inputs.size(0)):
                            try:
                                ...
                                gif_path = self.comparison_dir / f"transformer_{category}_{video_name}_seq{seq_idx}.gif"
...
def main():
    model_path = project_root / "models/Transformer_best.pth"
    test_data_path = project_root / "processed_data/test_sequences.h5"
    
    if not model_path.exists():
        raise FileNotFoundError(f"Model not found at {model_path}")
        
    tester = TransformerTester(str(model_path))
